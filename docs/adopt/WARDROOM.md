...
- `infra/cloudflare/wardroom.tf` gains, on the latchkey shape:
  ```hcl
  ...
  resource "cloudflare_pages_domain" "www" {
    for_each     = toset(["wardroom.id", "www.wardroom.id"])
    account_id   = var.account_id
  ...
  resource "cloudflare_dns_record" "www_site" {
    ...
    name     = each.key
    type     = "CNAME"
    content  = "wardroom-www.pages.dev"
    proxied  = true
    ttl      = 1
  }
  ```
...
- `.github/workflows/www.yml`: on push to main with `paths: [www/**]`
  ...
  `npx wrangler pages deploy dist --project-name wardroom-www --branch=main`
